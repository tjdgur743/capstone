...
cam=True
raspberry = True
yolo_model='yolov5/weights/yolov5s.onnx' if not cam else 'yolov5/weights/best.onnx'
...
classes=[0,1] if cam else [0,2] # car:2, truck:7, 67: phone
...
client_socket=connect_to_raspberry() if raspberry else False
...
source="resource/outcase_4.mp4"
parking_space_center=[119,247,375,501,628] # x좌표
coordinate_factor=50
if cam: #165 290 410 510
    parking_space = [
        [parking_space_center[0]-coordinate_factor, parking_space_center[0]+coordinate_factor, 220, 280], 
        [parking_space_center[1]-coordinate_factor, parking_space_center[1]+coordinate_factor, 220, 280], 
        [parking_space_center[2]-coordinate_factor, parking_space_center[2]+coordinate_factor, 220, 280], 
        [parking_space_center[3]-coordinate_factor, parking_space_center[3]+coordinate_factor, 220, 280],
        [parking_space_center[4]-coordinate_factor, parking_space_center[4]+coordinate_factor, 220, 280]
    ]
...
for frame_idx, (path, image, image0s, vid_cap, s) in enumerate(input): # Frames
    ...
    for i, detection in enumerate(pred):  # Deepsort on detections in this frame
        ...
        if detection is not None and len(detection):
            ...
            outputs = deepsort.update(xywhs.cpu(), confs.cpu(), clss.cpu(), image0, parked_list, cam, client_socket)
            ...
            for j, (output) in enumerate(outputs):
                ...
                # label = f'{id:.0f} {names[c]}'
                label = f'{id:.0f} {names[c]}/{elapsed_time:0.0f}/{output[10]}/{output[11]}'
                #label = f'{int(output[10]*(640/len(image0[0])))}, {int(output[11]*(640/len(image0)))}'
                ...
                for k in range(len(parked_list)):
                    ...
                    if parked_list[k] < 0 and names[c] == 'car' and parking_space[k][0] <= output[10] <= parking_space[k][1] and  parking_space[k][2] <= output[11] <= parking_space[k][3]:
                        if waiting_time == 0:
                        ...
                        elif elapsed_time - waiting_time > 20:
                            waiting_time = 0
                            ...
                            if raspberry:
                                client_socket.sendall(str(parked_list).encode())
                                client_socket.recv(99)                                                          
                if  names[c] == 'car' and int(id) not in parked_list and 5 < int(elapsed_time) < 40:
        ...
        if cam:
            ...
            for i in range(5): 
                if i == 0:
                    circle_x = parking_space_center[0]
                elif i == 1:
                    circle_x = parking_space_center[1]
                elif i == 2:
                    circle_x = parking_space_center[2]
                elif i == 3:
                    circle_x = parking_space_center[3]
                elif i == 4:
                    circle_x = parking_space_center[4]
                
                if parked_list[i] == 0:
                    cv2.circle(image0, (circle_x, circle_y), 10, (0,255,0), -1)
                    #cv2.circle(image0, (parking_space_center[i]-coordinate_factor, 90), 4, (0,255,0), -1)
                    #cv2.circle(image0, (parking_space_center[i]+coordinate_factor, 90), 4, (0,255,0), -1)
                elif parked_list[i] < 0:
                    cv2.circle(image0, (circle_x, circle_y), 10, (0,127,255), -1)
                    #cv2.circle(image0, (parking_space_center[i]-coordinate_factor, 90), 4, (0,127,255), -1)
                    #cv2.circle(image0, (parking_space_center[i]+coordinate_factor, 90), 4, (0,127,255), -1)
                elif parked_list[i] >= 1:
                    cv2.circle(image0, (circle_x, circle_y), 10, (0,0,255), -1)
                    #cv2.circle(image0, (parking_space_center[i]-coordinate_factor, 90), 4, (0,0,255), -1)
                    #cv2.circle(image0, (parking_space_center[i]+coordinate_factor, 90), 4, (0,0,255), -1)
